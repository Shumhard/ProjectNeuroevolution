...
namespace NeuralNetwork.Core.Implementations.NeuralNetworks
{
    internal class NeuralNetwork
    {
        internal List<ILayer> _layers;
        ...
        private double CalculateTotalError(List<double> outputs, int row)
        {
            ...
            {
                var error = Math.Pow(output - this._expectedResult[row][outputs.IndexOf(output)], 2);
                totalError += error;
            });
